...
func processDBCommand(conn *ws.Conn, deps Dependencies, payload []byte) error {
	...
	case "bigtable":
		var request dbrunner.BigtableQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid bigtable payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunBigtableQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "influx":
		var request dbrunner.InfluxQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid influx payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunInfluxQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "weaviate":
		var request dbrunner.WeaviateQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid weaviate payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunWeaviateQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "milvus":
		var request dbrunner.MilvusQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid milvus payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunMilvusQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "bigquery":
		var request dbrunner.BigQueryQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid bigquery payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunBigQueryQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "etcd":
		var request dbrunner.EtcdQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid etcd payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunEtcdQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "zookeeper":
		var request dbrunner.ZookeeperQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid zookeeper payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunZookeeperQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "kafka":
		var request dbrunner.KafkaQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid kafka payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunKafkaQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "nacos":
		var request dbrunner.NacosQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid nacos payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunNacosQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
	...
	case "rocketmq":
		var request dbrunner.RocketMQQueryRequest
		if err := json.Unmarshal(command.Payload, &request); err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
				"type":  "error",
				"error": "invalid rocketmq payload",
			})
		}
		ctx, cancel := context.WithCancel(context.Background())
		registerDBRequest(command.ID, cancel)
		defer finishDBRequest(command.ID)
		result, err := dbrunner.RunRocketMQQuery(ctx, request, deps.Config.DatabaseTimeout)
		cancel()
		if err != nil {
			return conn.WriteJSON(fiber.Map{
				"id":    command.ID,
