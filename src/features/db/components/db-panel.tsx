...
export function DbPanel(props: DbPanelProps) {
  ...
  const [sidebarView, setSidebarView] = createSignal<DbSidebarView>("connections");
  ...
  const [folderMenu, setFolderMenu] = createSignal<FolderMenuState | null>(null);
  const [folderAddMenuId, setFolderAddMenuId] = createSignal<string | null>(null);
  const [connectionMenu, setConnectionMenu] = createSignal<ConnectionMenuState | null>(null);
  const [connectionMoveMenuId, setConnectionMoveMenuId] = createSignal<string | null>(null);
  ...
  const [resultByTabId, setResultByTabId] = createSignal<Record<string, DbResultPayload>>({});
  ...
  const activeTab = createMemo(() =>
    workspace().activeTabId ? workspace().tabsById[workspace().activeTabId] ?? null : null
  );
  const activeConnection = createMemo(() =>
    activeTab() ? connectionMap().get(activeTab()!.connectionId) ?? null : null
  );
  const folderEntries = createMemo(() =>
    workspace().folders.map((folder) => ({
      folder,
      connections: workspace().connections.filter((connection) => connection.folderId === folder.id)
    }))
  );
  ...
  const filteredHistory = createMemo(() => {
    ...
    return workspace().history.filter((item) =>
      `${item.connectionName} ${item.query}`.toLowerCase().includes(normalizedFilter())
    );
  });
  ...
  async function openConnectionTab(connection: DbConnection, forceNew = false) {
    const existingId =
      !forceNew
        ? workspace().openTabIds.find((tabId) => workspace().tabsById[tabId]?.connectionId === connection.id) ?? null
        : null;
  ...
  async function closeOtherTabs(tabId: string) {
    await commitWorkspace((draft) => {
      const keepIds = draft.openTabIds.filter((id) => id === tabId || draft.pinnedTabIds.includes(id));
      draft.openTabIds = keepIds;
      draft.activeTabId = keepIds.includes(draft.activeTabId ?? "") ? draft.activeTabId : tabId;
  ...
  async function closeAllTabs() {
    await commitWorkspace((draft) => {
      const keepIds = draft.pinnedTabIds.filter((id) => draft.openTabIds.includes(id));
      draft.openTabIds = keepIds;
  ...
  async function closeTabsToDirection(tabId: string, direction: "left" | "right") {
    await commitWorkspace((draft) => {
  ...
  async function reorderTabsToEnd(fromId: string) {
    await commitWorkspace((draft) => {
      ...
      const moved = arrayMove(draft.openTabIds, fromIndex, draft.openTabIds.length - 1);
      const pinned = moved.filter((id) => draft.pinnedTabIds.includes(id));
      const unpinned = moved.filter((id) => !draft.pinnedTabIds.includes(id));
      draft.openTabIds = [...pinned, ...unpinned];
    });
  ...
  async function deleteFolder(folderId: string) {
    await commitWorkspace((draft) => {
      ...
      draft.connections = draft.connections.map((connection) =>
        connection.folderId === folderId ? { ...connection, folderId: null } : connection
      );
    });
  ...
  async function moveFolderDirection(folderId: string, direction: "up" | "down") {
    await commitWorkspace((draft) => {
      const orderedIds = reorderByDirection(draft.folders.map((folder) => folder.id), folderId, direction);
      const order = new Map(orderedIds.map((id, index) => [id, index]));
      draft.folders.sort((a, b) => (order.get(a.id) ?? 0) - (order.get(b.id) ?? 0));
    });
    setFolderMenu(null);
  }

  async function moveConnectionDirection(connection: DbConnection, direction: "up" | "down") {
    await commitWorkspace((draft) => {
      ...
      const original = new Map(draft.connections.map((item, index) => [item.id, index]));
  ...
  async function moveConnectionToFolder(connection: DbConnection, folderId: string | null) {
    await commitWorkspace((draft) => {
      const target = draft.connections.find((item) => item.id === connection.id);
      if (target) {
  ...
  async function deleteConnection(connectionId: string) {
    await commitWorkspace((draft) => {
      ...
      const removedTabIds = Object.values(draft.tabsById)
        ...
        .map((tab) => tab.id);
      draft.openTabIds = draft.openTabIds.filter((id) => !removedTabIds.includes(id));
      draft.pinnedTabIds = draft.pinnedTabIds.filter((id) => !removedTabIds.includes(id));
      draft.tabsById = Object.fromEntries(
        Object.entries(draft.tabsById).filter(([, tab]) => tab.connectionId !== connectionId)
      );
      if (draft.activeTabId && removedTabIds.includes(draft.activeTabId)) {
        draft.activeTabId = draft.openTabIds.at(-1) ?? null;
      }
      draft.favorites = draft.favorites.filter((item) => item.connectionId !== connectionId);
      draft.history = draft.history.filter((item) => item.connectionId !== connectionId);
    });
    setConnectionMenu(null);
    setConnectionMoveMenuId(null);
  }
  ...
  async function saveCurrentTab() {
    ...
    if (!tab || !connection) return;
    await commitWorkspace((draft) => {
      const targetConnection = draft.connections.find((item) => item.id === connection.id);
      const targetTab = draft.tabsById[tab.id];
      if (!targetConnection || !targetTab) return;
      targetConnection.name = targetConnection.name.trim() || getConnectionTypeLabel(targetConnection.kind);
  ...
  async function addCurrentQueryToFavorites() {
    ...
    await commitWorkspace((draft) => {
      const existing = draft.favorites.find(
        (item) => item.connectionId === connection.id && item.query.trim() === tab.query.trim()
      );
      if (existing) {
      ...
      draft.favorites = [createDbFavorite(connection, tab.query, tab.query.split("\n")[0]), ...draft.favorites].slice(0, 100);
    });
  ...
  async function removeFavorite(favoriteId: string) {
    await commitWorkspace((draft) => {
      draft.favorites = draft.favorites.filter((item) => item.id !== favoriteId);
    });
  }
  ...
  async function runCurrentTab() {
    ...
    try {
      ...
      await commitWorkspace((draft) => {
        draft.history = [createDbHistoryItem(connection, tab.query, "success"), ...draft.history].slice(0, 150);
      });
    } catch (error) {
      ...
      await commitWorkspace((draft) => {
        draft.history = [createDbHistoryItem(connection, tab.query, "error"), ...draft.history].slice(0, 150);
      });
    }
  }

  const tabItems = createMemo(() =>
    workspace().openTabIds
      .map((tabId) => {
    ...
    return (
      <div
        class={`theme-sidebar-item group flex min-w-0 w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left ${
          activeConnection()?.id === connection.id ? "theme-sidebar-item-active" : ""
        }`}
        onClick={() => {
          const existing = workspace().openTabIds.find((tabId) => workspace().tabsById[tabId]?.connectionId === connection.id);
          if (existing) {
        ...
        onContextMenu={(event) => {
          ...
          setConnectionMenu({ id: connection.id, x: event.clientX, y: event.clientY });
          setConnectionMoveMenuId(null);
          setFolderMenu(null);
          setHeaderMenuOpen(false);
        }}
      >
        <span class={`${getConnectionBadge(connection).class} shrink-0`}>{getConnectionBadge(connection).label}</span>
        <button class="min-w-0 flex-1 text-left">
          <p class="truncate text-[13px] font-medium" title={connection.name}>{connection.name}</p>
        </button>
        <div
          ...
          <button
            ...
            onClick={(event) => {
              ...
              setConnectionMenu((current) =>
                current?.id === connection.id ? null : { id: connection.id, x: 0, y: 0 }
              );
            }}
          >
  ...
  function renderConfigField(label: string, value: string, onInput: (value: string) => void, type = "text", placeholder?: string) {
    return (
      <label class="grid gap-1">
        <span class="theme-text-soft text-[11px] uppercase tracking-[0.16em]">{label}</span>
        <input
  ...
  function renderConnectionConfigForm(connection: DbConnection) {
    ...
    const portPlaceholder = connection.kind === "sqlserver" ? "1433" : connection.kind === "clickhouse" ? "8123" : connection.kind === "mysql" || connection.kind === "tidb" ? "3306" : "5432";
  ...
  function renderRedisResult(result: DbResultPayload) {
    ...
    if (Array.isArray(value)) {
      return (
        <div class="grid gap-2">
          <For each={value}>
            {(item, index) => (
              <div class="theme-code rounded-[18px] border px-3 py-2" style={{ "border-color": "var(--app-border)" }}>
                <p class="theme-text-soft text-[11px] uppercase tracking-[0.16em]">Item {index() + 1}</p>
                <pre class="mt-2 whitespace-pre-wrap break-all font-mono text-xs">{JSON.stringify(item, null, 2)}</pre>
              </div>
            )}
          </For>
        </div>
    ...
    if (value && typeof value === "object") {
      return (
        <div class="theme-code overflow-auto rounded-[18px] border" style={{ "border-color": "var(--app-border)" }}>
          <table class="min-w-full border-collapse text-sm">
            <tbody>
              <For each={Object.entries(value as Record<string, unknown>)}>
                {([key, item]) => (
                  <tr>
                    <td class="theme-kv-head border-b px-3 py-2 align-top font-medium" style={{ "border-color": "var(--app-border)" }}>{key}</td>
                    <td class="theme-kv-cell border-b px-3 py-2 align-top" style={{ "border-color": "var(--app-border)" }}>
                      <pre class="m-0 whitespace-pre-wrap break-all font-mono text-xs">{JSON.stringify(item, null, 2)}</pre>
                    </td>
                  </tr>
                )}
              </For>
    ...
    return (
      <div class="theme-code rounded-[18px] border p-3" style={{ "border-color": "var(--app-border)" }}>
        <pre class="m-0 whitespace-pre-wrap break-all font-mono text-xs">{String(value ?? "")}</pre>
      </div>
    );
  }
        ...
        <For each={documents}>
          {(document, index) => (
            <div class="theme-code rounded-[18px] border p-3" style={{ "border-color": "var(--app-border)" }}>
              ...
              <pre class="mt-2 whitespace-pre-wrap break-all font-mono text-xs">{JSON.stringify(document, null, 2)}</pre>
            </div>
          )}
        </For>
      </div>
  ...
  function renderResultView() {
    ...
    return (
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="flex items-center justify-between border-b px-3 py-2" style={{ "border-color": "var(--app-border)" }}>
          <div class="flex items-center gap-2">
            <button
              class={`rounded-lg px-2 py-1 text-[11px] font-medium transition ${
                resultView === "table" ? "bg-[var(--app-accent-soft)] text-[var(--app-accent)]" : "theme-text-soft hover:text-[var(--app-text)]"
              }`}
              onClick={() => setResultViewByTabId((current) => ({ ...current, [tab.id]: "table" }))}
            >
              Results
            </button>
            <button
              class={`rounded-lg px-2 py-1 text-[11px] font-medium transition ${
                resultView === "raw" ? "bg-[var(--app-accent-soft)] text-[var(--app-accent)]" : "theme-text-soft hover:text-[var(--app-text)]"
              }`}
              onClick={() => setResultViewByTabId((current) => ({ ...current, [tab.id]: "raw" }))}
            >
              Raw
            </button>
          </div>
          <div class="theme-text-soft text-xs">
            ...
            <Show when={execution.status === "success" && resultMeta}>{resultMeta}</Show>
          </div>
        </div>
        <div class="min-h-0 flex-1 overflow-auto p-3">
          <Show
            when={result}
            fallback={<div class="theme-text-soft text-sm">Run a query to see results.</div>}
          >
            <Show when={resultView === "raw" || result?.kind !== "sql"} fallback={
              <Show
                when={result?.kind === "sql" && result.data.columns && result.data.rows}
                fallback={
                  <div class="grid gap-3 md:grid-cols-2">
                    <div class="theme-code rounded-[18px] border px-4 py-3" style={{ "border-color": "var(--app-border)" }}>
                      <p class="theme-text-soft text-[11px] uppercase tracking-[0.16em]">Affected Rows</p>
                      <p class="theme-text mt-2 text-lg font-semibold">{result?.kind === "sql" ? result.data.affectedRows ?? 0 : 0}</p>
                    </div>
                    <div class="theme-code rounded-[18px] border px-4 py-3" style={{ "border-color": "var(--app-border)" }}>
                      <p class="theme-text-soft text-[11px] uppercase tracking-[0.16em]">Last Insert ID</p>
                      <p class="theme-text mt-2 text-lg font-semibold">{result?.kind === "sql" ? result.data.lastInsertId ?? 0 : 0}</p>
                    </div>
                  </div>
                }
              >
                    ...
                    <thead>
                      <tr>
                        <For each={result?.data.columns ?? []}>
                          {(column) => (
                            <th class="theme-kv-head border-b px-3 py-2 text-left font-medium" style={{ "border-color": "var(--app-border)" }}>
                              {column}
                            </th>
                          )}
                        </For>
                        ...
                        {(row) => (
                          <tr>
                            <For each={result?.data.columns ?? []}>
                              {(column) => (
                                <td class="theme-kv-cell border-b px-3 py-2 align-top" style={{ "border-color": "var(--app-border)" }}>
                                  <pre class="m-0 whitespace-pre-wrap break-all font-mono text-xs">{JSON.stringify(row[column], null, 2)}</pre>
                                </td>
                              )}
                            </For>
                          </tr>
              ...
              <Show when={resultView === "raw"} fallback={
                <Show when={result?.kind === "redis"} fallback={<Show when={result?.kind === "mongo"} fallback={
                  <div class="theme-code h-full overflow-auto rounded-[18px] border p-3" style={{ "border-color": "var(--app-border)" }}>
                    <pre class="m-0 whitespace-pre-wrap break-words font-mono text-xs">{raw}</pre>
                  </div>
                }>
  ...
  return (
    <>
      <WorkspaceSidebarLayout
        ...
        sidebar={
          <>
            <div class="mb-4 flex items-center justify-between border-b pb-3" style={{ "border-color": "var(--app-border)" }}>
                ...
                <div class="flex items-center gap-1">
                  <div class="relative" data-db-menu-root>
                    ...
                    <Show when={headerMenuOpen()}>
                      <div class="theme-panel-soft theme-menu-popover absolute right-0 top-6 z-20 min-w-[148px] border p-1" data-db-menu-root style={{ "border-color": "var(--app-border)" }}>
                        <button class="theme-sidebar-item w-full rounded-xl px-3 py-2 text-left text-sm" onClick={() => void addFolder()}>
                          Add Folder
                        </button>
                      </div>
                    </Show>
                  ...
                  <div class="relative" data-db-menu-root>
                    ...
                    <Show when={addConnectionMenuOpen()}>
                      <div class="theme-panel-soft theme-menu-popover absolute right-0 top-6 z-20 min-w-[172px] border p-1" data-db-menu-root style={{ "border-color": "var(--app-border)" }}>
            ...
            <div class="mb-3 grid grid-cols-3 gap-0 overflow-hidden rounded-lg border" style={{ "border-color": "var(--app-border)" }}>
              <button
            ...
            <Show when={sidebarView() === "connections"}>
                  ...
                  {(entry) => (
                    <div class="grid gap-1">
                      <div
                        ...
                        onContextMenu={(event) => {
                          event.preventDefault();
                          setFolderMenu({ id: entry.folder.id, x: event.clientX, y: event.clientY });
                          setHeaderMenuOpen(false);
                          setAddConnectionMenuOpen(false);
                          setFolderAddMenuId(null);
                          setConnectionMenu(null);
                      ...
                      >
                        ...
                        >
                          <svg class={`h-3 w-3 transition-transform ${isFolderExpanded(entry.folder.id) ? "rotate-90" : ""}`} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M6 4.5L9.5 8L6 11.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
                          </svg>
                        </button>
                        <button class="min-w-0 flex-1 text-left" title={entry.folder.name} onClick={() => toggleFolderExpanded(entry.folder.id)}>
                        ...
                        <div class={`relative shrink-0 transition-opacity ${folderMenu()?.id === entry.folder.id ? "opacity-100" : "opacity-0 group-hover:opacity-100 group-focus-within:opacity-100"}`} data-db-menu-root>
                          <button
                            class="traffic-dot-button inline-flex h-5 w-5 items-center justify-center rounded-full p-0"
                            title="Folder options"
                            onClick={(event) => {
                              event.stopPropagation();
                              setFolderMenu((current) => current?.id === entry.folder.id ? null : { id: entry.folder.id, x: 0, y: 0 });
                            }}
                          >
                            <ControlDot variant="menu" />
                          </button>
                        ...
                        >
                          <button
                            ...
                            onClick={(event) => {
                              event.stopPropagation();
                              setFolderAddMenuId((current) => current === entry.folder.id ? null : entry.folder.id);
                              setFolderMenu(null);
                            }}
                          >
                            <ControlDot variant="add" />
              ...
              <Show when={normalizedFilter() ? filteredConnectionsFlat().length === 0 : rootConnections().length === 0 && folderEntries().length === 0}>
            ...
            <Show when={sidebarView() === "favorites"}>
              <div class="grid gap-1">
                  ...
                  {(favorite) => {
                    ...
                    return (
                      <div class="theme-sidebar-item group flex items-start gap-2 rounded-lg px-2 py-2">
                        <span class={`${connection ? getConnectionBadge(connection).class : "theme-method-badge theme-method-default"} mt-0.5 shrink-0`}>
                          {connection ? getConnectionBadge(connection).label : "DB"}
                        </span>
                        <button class="min-w-0 flex-1 text-left" onClick={() => void applyFavorite(favorite)}>
                          <p class="truncate text-[13px] font-medium">{favorite.name}</p>
                          <p class="theme-text-soft mt-0.5 line-clamp-2 break-all text-[11px]">{favorite.query}</p>
                        </button>
                        <button
                ...
                <Show when={filteredFavorites().length === 0}>
                  <div class="theme-text-soft px-2 py-2 text-xs">No favorites yet</div>
                </Show>
              </div>
            </Show>
            ...
            <Show when={sidebarView() === "history"}>
              <div class="grid gap-1">
                <For each={filteredHistory()}>
                  {(item) => (
                    <button class="theme-sidebar-item flex items-start gap-2 rounded-lg px-2 py-2 text-left" onClick={() => void applyHistoryItem(item)}>
                      <span class={`mt-0.5 inline-flex h-2.5 w-2.5 shrink-0 rounded-full ${item.status === "error" ? "bg-[#ff5f57]" : "bg-[#26c73f]"}`} />
                      <div class="min-w-0 flex-1">
                        <p class="truncate text-[13px] font-medium">{item.connectionName}</p>
                        <p class="theme-text-soft mt-0.5 line-clamp-2 break-all text-[11px]">{item.query}</p>
                        <p class="theme-text-soft mt-1 text-[10px] uppercase tracking-[0.14em]">{new Date(item.createdAt).toLocaleString()}</p>
                      </div>
                    </button>
                  )}
                </For>
                <Show when={filteredHistory().length === 0}>
                  <div class="theme-text-soft px-2 py-2 text-xs">No history yet</div>
                </Show>
              </div>
            </Show>
          </>
          ...
          <Show when={tabItems().length > 0}>
            <div class="border-b" style={{ "border-color": "var(--app-border)" }}>
              <RequestTabsBar
                ...
                onTabOpen={(tabId) => void commitWorkspace((draft) => { draft.activeTabId = tabId; })}
                onTabClose={(tabId) => void closeTab(tabId)}
                onTabContextMenu={(tabId, event) => setTabMenu({ id: tabId, x: event.clientX, y: event.clientY })}
          ...
          <Show when={activeTab() && activeConnection()} fallback={<div class="flex-1 min-h-0" />}>
            <div class="border-b px-3 py-2" style={{ "border-color": "var(--app-border)" }}>
              <div class="grid gap-2 xl:grid-cols-[auto_minmax(180px,0.9fr)_auto_auto_auto]">
                ...
                <button class="theme-control h-8 rounded-md px-3 text-sm font-medium" onClick={() => void saveCurrentTab()}>
                  Save
                </button>
            ...
            <div class="grid min-h-0 flex-1 grid-rows-[minmax(0,1fr)_1px_minmax(0,1fr)]">
              <div class="min-h-0 overflow-hidden p-3">
                <div class="mb-2 flex items-center justify-between">
                  ...
                  <p class="theme-text-soft text-xs">{getConnectionTypeLabel(activeConnection()!.kind)}</p>
                </div>
                <textarea
                  class="theme-input h-full min-h-[220px] w-full resize-none rounded-[18px] px-3 py-2 font-mono text-sm leading-6"
                  value={activeTab()!.query}
                  onInput={(event) => void updateActiveQuery(event.currentTarget.value)}
                />
              </div>
      ...
      <Show when={folderMenu()}>
        ...
        >
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void renameFolder(workspace().folders.find((folder) => folder.id === folderMenu()!.id)!)}>Rename</button>
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void moveFolderDirection(folderMenu()!.id, "up")}>Move Up</button>
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void moveFolderDirection(folderMenu()!.id, "down")}>Move Down</button>
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm text-[#ff3b30]" onClick={() => void deleteFolder(folderMenu()!.id)}>Delete</button>
        </div>
      </Show>

      <Show when={connectionMenu()}>
        {() => {
          const connection = workspace().connections.find((item) => item.id === connectionMenu()!.id)!;
          return (
            <div
              class="theme-panel-soft fixed z-[300] inline-grid auto-cols-max overflow-hidden rounded-[18px] border p-1.5 shadow-[0_18px_45px_rgba(15,23,42,0.18)]"
              data-db-menu-root
            ...
            >
              ...
              <div class="relative" data-db-menu-root>
                <button
                  ...
                  onClick={(event) => {
                    event.stopPropagation();
                    setConnectionMoveMenuId((current) => current === connection.id ? null : connection.id);
                  }}
                >
                  <span>Move to</span>
                ...
                <Show when={connectionMoveMenuId() === connection.id}>
                  <div
                    ...
                    style={{ "border-color": "var(--app-border)" }}
                  >
                    <button class="theme-sidebar-item w-full rounded-xl px-3 py-2 text-left text-sm" onClick={() => void moveConnectionToFolder(connection, null)}>Root</button>
                    <For each={workspace().folders}>
                      {(folder) => (
                        <button class="theme-sidebar-item w-full rounded-xl px-3 py-2 text-left text-sm" onClick={() => void moveConnectionToFolder(connection, folder.id)}>
                          {folder.name}
                        </button>
                      )}
                    </For>
              ...
              <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm text-[#ff3b30]" onClick={() => void deleteConnection(connection.id)}>Delete</button>
            </div>
          );
        }}
      </Show>
      ...
      <Show when={tabMenu()}>
        ...
        >
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void togglePinnedTab(tabMenu()!.id)}>
            {workspace().pinnedTabIds.includes(tabMenu()!.id) ? "UnPin" : "Pin"}
          </button>
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void closeOtherTabs(tabMenu()!.id)}>Close Others</button>
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void closeAllTabs()}>Close All</button>
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void closeTabsToDirection(tabMenu()!.id, "right")}>Close Right</button>
          <button class="theme-sidebar-item whitespace-nowrap rounded-xl px-3 py-2 text-left text-sm" onClick={() => void closeTabsToDirection(tabMenu()!.id, "left")}>Close Left</button>
        </div>
      </Show>
    </>
  );
