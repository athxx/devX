...
import { defaultSyncSettings, type SyncProviderType, type SyncSettings } from "../types";
...
function FieldLabel(props: { label: string; hint?: string; children: JSX.Element }) {
  return (
    <label class="grid gap-2">
      <span class="theme-text text-sm font-medium">{props.label}</span>
      {props.hint ? <span class="theme-text-soft text-xs leading-5">{props.hint}</span> : null}
      {props.children}
    </label>
  );
}
...
export function SyncPanel() {
  const [settings, setSettings] = createSignal<SyncSettings>(defaultSyncSettings);
      ...
      <div class="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_320px]">
        <div class="grid gap-4">
          <div class="grid gap-4 md:grid-cols-3">
            ...
            <FieldLabel label="Auto Sync Interval" hint="Background sync cadence in seconds.">
              <input
                class="theme-input rounded-2xl px-3 py-3"
                min="15"
                step="5"
                type="number"
                value={String(Math.max(15, Math.round(settings().syncIntervalMs / 1000)))}
                onInput={(event) =>
                  updateSettings((current) => ({
                    ...current,
                    syncIntervalMs: Math.max(15, Number(event.currentTarget.value) || 30) * 1000
                  }))
                }
              />
            </FieldLabel>
            ...
            <label class="theme-control flex items-center justify-between rounded-2xl px-4 py-3">
              <div>
                ...
                <p class="theme-text-soft mt-1 text-xs">Connected providers sync on a timer.</p>
              </div>
              <button
                class={`inline-flex h-7 w-11 items-center rounded-full px-1 transition ${
                  settings().autoSync ? "bg-[var(--app-accent)]" : "bg-[var(--app-panel-strong)]"
                }`}
                type="button"
                onClick={() =>
                  updateSettings((current) => ({
          ...
          <Switch>
            <Match when={settings().provider === "dropbox"}>
              <div class="grid gap-4 md:grid-cols-2">
                <FieldLabel label="Dropbox Access Token" hint="Temporary token login for the first sync version.">
                  <input
                    class="theme-input rounded-2xl px-3 py-3"
                    placeholder="sl.B..."
                    type="password"
                ...
                <FieldLabel label="Remote Path" hint="Dropbox path used to save the snapshot JSON.">
                  <input
                    class="theme-input rounded-2xl px-3 py-3"
            ...
            <Match when={settings().provider === "onedrive"}>
              <div class="grid gap-4 md:grid-cols-2">
                <FieldLabel label="OneDrive Access Token" hint="Microsoft Graph token for reading and writing snapshots.">
                  <input
                    class="theme-input rounded-2xl px-3 py-3"
                    placeholder="EwB..."
                    type="password"
                ...
                <FieldLabel label="Remote Path" hint="Path inside the app folder or drive root.">
                  <input
                    class="theme-input rounded-2xl px-3 py-3"
            ...
            <Match when={settings().provider === "webdav"}>
              <div class="grid gap-4 md:grid-cols-2">
                <FieldLabel label="WebDAV Endpoint" hint="Base endpoint without the final file path.">
                  <input
                    class="theme-input rounded-2xl px-3 py-3"
                ...
                <FieldLabel label="Remote Path" hint="JSON snapshot file path in WebDAV.">
                  <input
                    class="theme-input rounded-2xl px-3 py-3"
            ...
            <Match when={settings().provider === "none"}>
              <div class="theme-control rounded-3xl p-4">
                ...
                <p class="theme-text-muted mt-2 text-sm leading-6">
                  Workspace data stays in IndexedDB. This is the safest option while the rest of the
                  product is still being built out.
                </p>
              </div>
            </Match>
          </Switch>
        ...
        <div class="grid gap-3">
          <div class="theme-control rounded-3xl p-4">
            ...
            <p class="theme-text mt-2 text-lg font-semibold">{settings().status}</p>
          </div>
          <div class="theme-control rounded-3xl p-4">
            <p class="theme-text-soft text-xs uppercase tracking-[0.18em]">Provider</p>
            <p class="theme-text mt-2 text-lg font-semibold capitalize">{settings().provider}</p>
          </div>
          <div class="theme-control rounded-3xl p-4">
            <p class="theme-text-soft text-xs uppercase tracking-[0.18em]">Last Sync</p>
            <p class="theme-text mt-2 text-sm font-medium">{formatTime(settings().lastSyncedAt)}</p>
          </div>
          <div class="theme-control rounded-3xl p-4">
            <p class="theme-text-soft text-xs uppercase tracking-[0.18em]">IndexedDB Snapshot</p>
            <p class="theme-text mt-2 text-sm font-medium">{formatTime(localUpdatedAt())}</p>
          </div>
